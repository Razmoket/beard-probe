...
/**
 * Provides methods necessary to build connection with EPP Server using a SSL-TLS socket. It only overrides the connect() method from EPPTransportTCP
 * since the remaining socket operations are identical to unencrypted sockets.
 * <P>
 * This class uses Sun's JSSE to establish a secure connection with the server. It makes use of the RTK property "ssl.props.location" to retrieve the
 * ssl.properties. This properties file contains information necessary to locate the java keystore, the names of the keys and certificates contained
 * inside it and the necessary passwords to access the keystore. The epp-rtk/java/ssl director contains more information regarding the data required
 * to create a keystore and the steps to do so.
 * <P>
 * If the user does not wish to use the default Sun JSSE but rather a third party SSL implemention, then the socket connection should be established
 * externally and an instance of EPPTransportTCP should be created using the connected socket.
 * <P>
...
public class EPPTransportTCPTLS extends EPPTransportTCP {
	...
	/**
	 * Connects to the Server using previously set Hostname and port. If connection has been already established, the operation will be ignored. The
	 * method also sets the SO timeout.
	 * 
	...
	public void connect() throws SocketException, IOException, UnknownHostException, EPPTransportException {
		...
		if (!preset_) {
			...
			if (ssl_props_location == null || ssl_props_location.length() == 0) { throw new IOException("No ssl props location specified"); }
			...
			try {
				...
				la_session = ((SSLSocket) socket_to_server_).getSession();
			} catch (Exception e) {
				System.out.println("socket_to_server_=" + socket_to_server_);
				System.out.println("la_session=" + la_session);
				System.out.println("la_session.getProtocol()=" + la_session.getProtocol());
			...
			if (socket_to_server_ == null || la_session.getProtocol().equals("NONE")) { 
				...
				throw new EPPTransportException(
			        "Failed to establish secure connection to server.  Perhaps a bad certificate? -- use -Djavax.net.debug=all to see errors."); }
		}
