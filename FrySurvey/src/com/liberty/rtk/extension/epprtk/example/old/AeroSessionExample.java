...
package com.liberty.rtk.extension.epprtk.example.old;
...
public class AeroSessionExample {
	...
	public void login() throws epp_Exception, IOException, epp_XMLException,
			EPPTransportException {
		epp_client = new EPPClient(epp_host_name, epp_host_port, epp_client_id,
				epp_password);
		...
		/*
		 * Uncomment following line if you don't want to send RTK version number
		 * on Login. Although Liberty RTK recomends to use this extension tag on
		 * Login request.
		 */
	...
	private void createContactWithMaintainerURL(String contact_id, String url)
			throws epp_Exception, epp_XMLException, IOException {
		...
		System.out
				.print("Dear registrant, please enter a passphrase for the new registrant contact(min 6, max 16): ");
		...
		contact_create = (EPPContactCreate) epp_client
				.processAction(contact_create);

		epp_ContactCreateRsp contact_create_response = contact_create
				.getResponseData();
		System.out.println("ContactCreate results: contact id ["
				+ contact_create_response.getId() + "]");
	}

	private void updateContactWithMaintainerURL(String contact_id, String url)
			throws epp_Exception, epp_XMLException {
		...
		contact_update = (EPPContactUpdate) epp_client
				.processAction(contact_update);

		epp_ContactUpdateRsp contact_update_response = contact_update
				.getResponseData();
		epp_Response response = contact_update_response.getRsp();
		epp_Result[] results = response.getResults();
		System.out.println("ContactUpdate results: [" + results[0].m_code
				+ "] [" + results[0].m_msg + "]");
	}

	private void verifyMaintainerURL(String contact_id) throws epp_Exception,
			epp_XMLException {
		...
		epp_ContactInfoRsp contact_info_response = contact_info
				.getResponseData();

		// display result
		epp_Response response = contact_info_response.getRsp();
		epp_Result[] results = response.getResults();
		System.out.println("ContactInfo results: [" + results[0].m_code + "] ["
				+ results[0].m_msg + "]");
		...
		if (extensionStrings != null) {
			...
			System.out.println("Maintainer URL: ["
					+ aero_contact_extension.getMaintainerUrl() + "]");
		} else {
			System.out
					.println("Contact Info response contained no extension!!!");
		}
	}

	/*
	 * ENS contact info (ENS contact is a special contact which owned by SITA
	 * but assigned to registrant
	 * 
	 * First, the registrar should check if the given ens_auth_id1 is valid in
	 * the registry. If it does not, we'll quit this session example.
	 */
	private void verifyENSContact(String ens_auth_id, epp_AuthInfo ens_auth_info)
			throws epp_Exception, epp_XMLException {
		System.out.println("Creating the Contact Info command");
		...
		if (extensionStrings != null) {
			AeroContactN aero_contact_extension = new AeroContactN();
			aero_contact_extension.fromXML(extensionStrings[0]);
			System.out.println("RegistrantGroup ["
					+ aero_contact_extension.getEnsInfo().getRegistrantGroup()
					+ "]");
			System.out.println("EnsO  ["
					+ aero_contact_extension.getEnsInfo().getEnsO() + "]");
			System.out.println("RequestType  ["
					+ aero_contact_extension.getEnsInfo().getRequestType()
					+ "]");
			System.out.println("RegistrationType  ["
					+ aero_contact_extension.getEnsInfo().getRegistrationType()
					+ "]");
			System.out.println("CredentialsType  ["
					+ aero_contact_extension.getEnsInfo().getCredentialsType()
					+ "]");
			System.out.println("CredentialsValue  ["
					+ aero_contact_extension.getEnsInfo().getCredentialsValue()
					+ "]");
			System.out.println("CodeValue  ["
					+ aero_contact_extension.getEnsInfo().getCodeValue() + "]");
			System.out.println("UniqueIdentifier  ["
					+ aero_contact_extension.getEnsInfo().getUniqueIdentifier()
					+ "]");
			System.out.println("LastCheckedDate  ["
					+ aero_contact_extension.getEnsInfo().getLastCheckedDate()
					+ "]");
		} else {
			System.out
					.println("Contact Info response contained no extension!!!");
	...
	private void checkDomain() throws epp_Exception, epp_XMLException {
		...
		domain_check_request.setNames(EPPXMLBase
				.convertListToStringArray(domain_list));
		...
		epp_DomainCheckRsp domain_check_response = domain_check
				.getResponseData();
		epp_CheckResult[] check_results = domain_check_response.getResults();
		System.out.println("DomainCheck results: domain [" + domain_name
				+ "] available? ["
		...
		if (EPPXMLBase.getAvailResultFor(check_results, domain_name) == null
			...
			System.out
					.println("domain "
							+ domain_name
							+ " exists in .aero registry, please choose another domain");
			System.exit(1);
		}
	}

	private boolean isContactAvailable(String contact_id) throws epp_Exception,
			epp_XMLException {
		...
		contact_check_request.setIds(EPPXMLBase
				.convertListToStringArray(contact_list));

		EPPContactCheck contact_check = new EPPContactCheck();
		contact_check.setRequestData(contact_check_request);

		contact_check = (EPPContactCheck) epp_client
				.processAction(contact_check);

		epp_ContactCheckRsp contact_check_response = contact_check
				.getResponseData();

		epp_CheckResult[] check_results = contact_check_response.getResults();
		System.out
				.println("ContactCheck results: contact ["
						+ contact_id
						+ "] available? ["
		...
		if (EPPXMLBase.getAvailResultFor(check_results, contact_id) != null) {
			return EPPXMLBase.getAvailResultFor(check_results, contact_id)
					.booleanValue();
		}
	...
	private void createContact(String contact_id) throws epp_Exception,
			IOException, epp_XMLException {
		System.out.println("Creating the Contact Create command");
		epp_ContactCreateReq contact_create_request = new epp_ContactCreateReq();
	...
	private void createDomainPending(String domain) throws epp_Exception,
			IOException, epp_XMLException {
		System.out
				.println("Creating the Domain Create command -- demostrating pending create in .AERO");
		...
		domain_contacts.add(new epp_DomainContact(epp_DomainContactType.TECH,
				contact_id2));
		domain_contacts.add(new epp_DomainContact(epp_DomainContactType.ADMIN,
				contact_id1));
		domain_contacts.add(new epp_DomainContact(
				epp_DomainContactType.BILLING, contact_id2));

		domain_create_request.setContacts((epp_DomainContact[]) EPPXMLBase
				.convertListToArray((new epp_DomainContact()).getClass(),
						domain_contacts));

		System.out
				.print("Dear registrant, please enter a passphrase for your new domain:(min 6, max 16) ");
		...
		domain_create = (EPPDomainCreate) epp_client
				.processAction(domain_create);
	...
	private void createDomain() throws epp_Exception, IOException,
			epp_XMLException {
	...
	private void updateDomainENSConect() throws epp_Exception, IOException,
			epp_XMLException {
		...
		domain_update = (EPPDomainUpdate) epp_client
				.processAction(domain_update);
		// or, alternatively, this method can be used...
		// domain_update.fromXML(epp_client.processXML(domain_update.toXML()));

		epp_DomainUpdateRsp domain_update_response = domain_update
				.getResponseData();
		epp_Response response = domain_update_response.m_rsp;
		epp_Result[] results = response.m_results;
		System.out.println("DomainUpdate results: [" + results[0].m_code
				+ "] [" + results[0].m_msg + "]");
	}

	private void getDomainInfo(String domain_name) throws epp_XMLException,
			epp_Exception {
		...
		for (int i = 0; i < domain_info_response.getStatus().length; i++) {
			System.out.println("\tstatus["
					+ i
					+ "] string ["
					+ EPPDomainBase.domainStatusToString(domain_info_response
							.getStatus()[i].getType()) + "]");
		...
		if (extensionStrings != null) {
			...
			System.out.println("ENS_Auth_ID ["
					+ aero_domain_extension.getEnsAuthID() + "]");
		} else {
			System.out
					.println("Domain Info response contained no extension!!!");
		}
	}
	...
	private epp_AuthInfo getUserInputAuthInfo() throws IOException {
		...
		BufferedReader buffed_reader = new BufferedReader(
